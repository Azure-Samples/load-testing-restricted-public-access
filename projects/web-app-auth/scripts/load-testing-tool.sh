...
if [[ "${ACTION}" == "deploy" ]] ; then
    ...
    updateConfigurationFile "${CONFIGURATION_FILE}" "AZURE_RESOURCE_APP_INSIGHTS_INSTRUMENTATION_KEY" "${AZURE_RESOURCE_APP_INSIGHTS_INSTRUMENTATION_KEY}"      
...
if [[ "${ACTION}" == "deployservices" ]] ; then
    ...
    if [ ${USE_STATIC_WEB_APP} == true ]; then
        ...
        TEMP_DIR=$(mktemp -d)        
        cmd="swa deploy  \"$SCRIPTS_DIRECTORY/../../../projects/web-app-auth/src/ts-web-app/build\" --deployment-token \"${STATIC_WEB_APP_TOKEN}\" --env production --verbose=silly  2> ${TEMP_DIR}/tmp.err "       
        printProgress "$cmd"
        eval "$cmd" || true
        if [ -s "${TEMP_DIR}/tmp.err" ]; then
            printError "An error may occurred in the swa command line, we don't stop the process as it's maybe a false error generated by the swa CLI:"            
            cat "${TEMP_DIR}/tmp.err"
        fi
    else
